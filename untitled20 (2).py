...
class_base_path = os.path.join(extract_path, "hoa_mono")
    ...
    html_result = f"""
        <div>
            <img src="data:image/png;base64,{img_base64}"
                 style="max-width:350px; border-radius:15px; box-shadow:0 0 15px #FF4081;">
            <div style="margin-top:15px; font-size:24px; font-weight:bold; color:#3F51B5; text-shadow:1px 1px #ccc;">
...
# ===== HTML Giao diện (theo style đoạn 2) =====
display(HTML("""
<!DOCTYPE html>
<html lang="vi">
<head>
<meta charset="UTF-8">
<title>Dự đoán hoa 🌸</title>
<link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;700&family=Pacifico&display=swap" rel="stylesheet">
<style>
    body { font-family: 'Roboto', sans-serif; background: linear-gradient(135deg, #fdfbfb 0%, #ebedee 100%); text-align:center; padding:40px; }
    h1 { font-family: 'Pacifico', cursive; color: #FF4081; font-size: 48px; margin-bottom: 30px; text-shadow: 2px 2px #ffd6e0;}
    button { margin-top: 20px; padding: 12px 30px; font-size:18px; border:none; border-radius:10px; background: linear-gradient(45deg, #FF4081, #F50057); color:white; cursor:pointer; font-weight:bold; box-shadow: 2px 2px 10px rgba(0,0,0,0.2); transition: all 0.3s ease; }
    button:hover { transform: translateY(-3px); box-shadow: 4px 4px 15px rgba(0,0,0,0.3); }
    #resetBtn { background: linear-gradient(45deg, #FF5722, #FF7043); display:none; }
    #result { margin-top:25px; font-size:24px; font-weight:bold; color:#3F51B5; text-shadow: 1px 1px #ccc; }
    #top3 { margin-top:15px; font-size:18px; color:#555; }
</style>
</head>
<body>
<h1>🌸 Dự đoán loài hoa 🌸</h1>

<button onclick="google.colab.kernel.invokeFunction('predict_money', [], {});">Chọn ảnh & Dự đoán</button>
<button id="resetBtn" onclick="location.reload();">Dự đoán ảnh khác</button>
